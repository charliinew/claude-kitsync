...
argument-hint: "[-a] [-s] [-e] [-b] [-i] [-t] [-r <task-id>] <task description>"
---
...
| `-t` | `--team` | Team mode: spawn Agent Teams for parallel multi-agent execution |
...
/apex -t implement user auth  # Spawn teammates for parallel execution
/apex -t -a -s implement user auth  # Team + auto + save
```
</examples>
...
| `{team_mode}`           | boolean | Spawn Agent Teams for parallel multi-agent execution   |
| `{team_config}`         | object  | Team composition: roles, spawn prompts, display mode   |
